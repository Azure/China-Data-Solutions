...
(Get-Content -Path $parametersFilePath) -replace("{{Location}}", $resourceGroupLocation) -replace("{{Name}}", $VMName.ToLower()) -replace("{{VMUsername}}", $VMUsername) -replace("{{Password}}", $VMPassword) -replace("{{ResourceGroup}}", $resourceGroupName) -replace("{{DBID}}", [Convert]::ToString([guid]::NewGuid()))| Set-Content $newParametersPath
