...
class TopicDictClassifier:
    ...
    def analyse_sentence(self, sentence, runout_filepath=None, simple_filepath=None, print_show=False):
        # 情感分析整体数据结构
        comment_analysis = {}
        ...
        for i in range(len(the_clauses)):
            ...
            self.__write_runout_file(runout_filepath, "Tclause:" + str(i) + "\n")
            self.__write_runout_file(runout_filepath, the_clauses[i] + "\n")
            self.__write_runout_file(runout_filepath, "clause sentimence:" + str(sub_clause['score']))
            seg_result = sub_clause['seg_result']
            for j in range(len(seg_result)):
                if len(seg_result[j].word) < 2:
                    continue
                if seg_result[j].flag in ['n', 'nr', 'ns', 'nt', 'nz', 'nl', 'ng', 'vd', 'vn', 'ad', 'an', 'ag', 'al',
                                          'eng']:
                    m_hotkey[seg_result[j].word] = 1
            for topic in sub_clause['topic']:
                if sub_clause['topic'][topic] == 0:
                    continue
                if topic in m_topic:
                    m_topic[topic] += sub_clause['topic'][topic] * sub_clause['score']
                else:
                    m_topic[topic] = sub_clause['topic'][topic] * sub_clause['score']
                self.__write_runout_file(runout_filepath,
                                         "topic:" + topic + " times:" + str(sub_clause['topic'][topic]) + "\n")
        l_topic = sorted(m_topic.items(), key=lambda d: d[1], reverse=True)
        topic_result = ""
        print l_topic
        if len(l_topic) > 0:
            topic_result = str(l_topic[0][0])
            comment_analysis["topic"] = topic_result
        if runout_filepath is not None and simple_filepath is not None:
            # 将整句写进运行输出文件，以便复查
            sentenceMd5 = hashlib.md5()
            sentenceMd5.update(sentence)
            sentence_Digest = sentenceMd5.hexdigest()
            self.__write_runout_file(simple_filepath, "\nMD5:" + sentence_Digest + "\n" + "content:" + sentence + "\n")
            self.__write_runout_file(simple_filepath, "topic:" + topic_result + "\n")
            self.__write_runout_file(simple_filepath, "sentimence:" + str(comment_analysis["score"]) + "\n")

            self.__write_runout_file(runout_filepath, "\nMD5:" + sentence_Digest + "\n" + "content:" + sentence + "\n")
            self.__write_runout_file(runout_filepath, "\n" + sentence + '\n')
            self.__write_runout_file(runout_filepath, "topic:" + topic_result + "\n")
        ...
        for key in m_hotkey:
            ...
            if cnt < len(m_hotkey):
                js_res += ''','''
        # js_res+= '''],"topic":"'''+topic_result+'''","sentimence":''' + str(comment_analysis["score"])+'''}'''
        # return js_res
        return topic_result
        # else:
        # return ""
    ...
    def __analyse_clause(self, the_clause, runout_filepath, print_show):
        ...
        sub_clause["seg_result"] = seg_result
        ta = TopicAnalysis()
        sub_clause["topic"] = ta.get_each_hit_num_in_dict(seg_result)
        if print_show:
        ...
        if runout_filepath is not None:
            ...
            self.__write_runout_file(runout_filepath, str(seg_result).encode('utf-8') + '\n')
        if print_show:
    ...
    def __output_analysis(self, comment_analysis, runout_filepath=None):
        output = ""
        for i in range(len(comment_analysis) - 2):
