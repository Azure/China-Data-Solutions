...
namespace MediaMonitoring.Controllers
{
    ...
    using Filter = DataAccessLayer.DataModels.Filters.Filter;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var signedUser = this.UserManager.FindByEmail(model.Email);
            if (signedUser == null)
            {
                this.ModelState.AddModelError("", "邮件地址或密码错误。");
                return this.View(model);
            }
            var result =
                await
                this.SignInManager.PasswordSignInAsync(signedUser.UserName, model.Password, model.RememberMe, false);
            switch (result)
            {
                ...
                default:
                    this.ModelState.AddModelError("", "邮件地址或密码错误。");// "Invalid login attempt.");
                    return this.View(model);
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (this.ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var filter = new Filter { Name = "Keywords", Value = cuser.Name };
        ...
        private void AddErrors(IdentityResult result)
        {
            ...
            {
                Regex regex1 = new Regex("Name ", RegexOptions.IgnoreCase);
                Regex regex2 = new Regex("Email ", RegexOptions.IgnoreCase);
                Regex regex3 = new Regex("is already taken", RegexOptions.IgnoreCase);
                var err = error;
                if (regex1.IsMatch(error) )
                {
                    err= regex1.Replace(error, "用户名");
                }
                if (regex2.IsMatch(error))
                {
                    err = regex2.Replace(error, "邮件地址");
                }
                if (regex3.IsMatch(error))
                {
                    err = regex3.Replace(error, "已经被使用");
                }
